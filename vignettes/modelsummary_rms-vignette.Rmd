...
XXXX it is just to make sure the knitted document uses the most recent version of rmsMD XXXXX
...
Here is a simple example using a linear regression model ("ordinary least squares"; OLS). The example data being used here is the built-in survey dataset from the MASS package.
...
## Exponential Coefficients (including hazard ratios and odds ratios)
...
Restricted Cubic Splines (RCS) are a flexible modelling tool used to capture non-linear relationships between predictors and outcomes. In medicine, many continuous variables (e.g. age, blood pressure, or biomarker levels) do not have a strictly linear association with the outcome of interest. A key highlight of the **rms** package is the ability to analyse variables using RCS.
...
- **Display an overall p-value for the spline terms using the `rcs_overallp` option.**  
  When this option is set to `TRUE` (which is the default), the function computes a single p-value that tests the overall significance of the spline terms. This overall p-value provides insight into whether the relationship between the predictor and the dependent variable is significant.
