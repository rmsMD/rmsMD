...
The **rmsMD** package is designed to report and summarise models that include RCS terms. Here is an example model predicting occurence of complications after surgery (binary), with the continuous variables age and BMI modelled using restricted cubic splines with 4 knots:
...
P values for restricted cubic spline terms in these outputs represent the whether there is an association between the predictor and outcome (which can be *either* linear or non-linear). We recommend that these are then plotted using the `ggrmsMD` function, shown below, to assess the nature of these relationships.
